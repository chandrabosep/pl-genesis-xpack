...
- **Pay at install**: Users pay with SUI (on Sui network), USDC (on supported EVM chains and Starknet), or FLOW (on Flow EVM chain) when they run `npm install`. Funds go straight to the author’s wallet and the install completes automatically, with no separate checkout or confusing flows.
...
- **Multi-network payments**: Accept SUI on Sui, USDC on Starknet and EVM chains (including Base Sepolia), and FLOW on Flow EVM chain. Expand your reach to users across leading blockchain ecosystems.

...
export async function GET(request: NextRequest) {
	try {
		...
		const address = isAnyChain ? unifiedReceiveAddress : attempt.project.paymentAddress;
		...
		return NextResponse.json({
			...
			paymentUri: firstOption?.paymentUri,
			chainId: firstOption?.chainId ?? paymentChainId(),
			currency: PAYMENT_TOKEN_SYMBOL,
			tokenAddress: firstOption?.tokenAddress,
			amountUnits: amountUnits?.toString(),
			pricingModel: pricingModel ?? undefined,
			githubUsername: attempt.githubUsername ?? undefined,
			githubUserId: attempt.githubUserId ?? undefined,
			receiveMode: isAnyChain ? "any_chain" : "base",
